...
class AuthorHWDataset(Dataset):
    def __init__(self, dirPath, split, config):
        ...
        # Read train.txt instead of XML
        train_file = os.path.join(dirPath, 'train.txt')
        ...
        for idx, line in enumerate(lines):
            ...
            author = "unknown"  # No author data in train.txt
            self.author_list.add(author)
            self.max_char_len = max(self.max_char_len, len(text))
            self.authors[author].append((img_path, text))
            # self.lineIndex.append((author, len(self.authors[author]) - 1))
            ...
            for author,lines in self.lineIndex:
                for line in lines:
                    ...
                    if not os.path.exists(fg_path):
                        img = cv2.imread(img_path,0) #read as grayscale, crop line
        ...
        self.max_stretch = 0.4
        self.max_rot_rad = 45 / 180 * 3.14159
    ...
    def __getitem__(self, idx):
        if type( self.augmentation) is str and 'affine' in self.augmentation:
            strech = (self.max_stretch*2)*np.random.random() - self.max_stretch +1
        ...
        for author,line in alines:
            ...
            # print("HELLOOOOOOOOOO",self.authors[author][line])
            img_path, gt = self.authors[author][line]
